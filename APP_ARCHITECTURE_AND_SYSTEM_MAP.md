...
- Implements Google ML Kit Text Recognition with Adaptive Cropped ROI Processing.
- Once the initial frame locates "Price Now" and numeric rows, an active ROI window tracks the metrics bounding box, reducing ML Kit recognition time from ~100-250ms down to ~15-35ms.
- Self-recovering and drift-proof: Automatically falls back to full-frame scanning whenever the phone moves or angle shifts.
- Uses regex patterns to identify live numbers: price quotes, percentage strings (`+1.24%`, `-0.85%`).
