...
object TradingOutputParser {
    ...
    fun calculate5_60_1dPercentage(
        ...
        val v1d = if (has1d) (val1d ?: 0.0) else 0.0
        val isAllBullish = (val5m ?: 0.0) > 0 && (val60m ?: 0.0) > 0 && (!has1d || v1d > 0)
        val isAllBearish = (val5m ?: 0.0) < 0 && (val60m ?: 0.0) < 0 && (!has1d || v1d < 0)
    ...
    fun evaluateDailyContext(val1d: Double?): DailyContext {
        if (val1d == null || !isValidMetricValue(val1d)) {
    ...
    fun evaluateDailyContextMatrix(
        ...
        if (!isValidMetricValue(val1d) || val1d == null || abs(val1d) < DAILY_ACTIVE_THRESHOLD) {
